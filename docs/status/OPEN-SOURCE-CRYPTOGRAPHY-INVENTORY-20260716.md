...
| Halo2 over Pasta | vendored `halo2_proofs 0.3.2` fork plus `pasta_curves 0.5`, `halo2_gadgets 0.5` | Asset-Orchard swap and private-egress proofs; Orchard action verification | Active circuits use `k=15`; embedded parameters and VK artifacts have hash/fingerprint checks. Exact upstream licenses, commit, normalized patch hash and a fail-closed verifier are included; specialist circuit review remains required before real value. |
...
| `P1-LICENSE-01` | Baseline vendored Halo2 lacked complete license/provenance | Fixed locally with exact licenses, upstream commit and normalized patch verifier |
