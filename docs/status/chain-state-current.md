...
!!! note "Freshness"
    ...
    An authenticated, read-only probe of all six validators completed at the
    timestamp above. It observed the state documented here. This is fresher than
    the committed activation packet, but it is still a point-in-time observation;
    re-probe the fleet before making a later “right now” claim.

## State Summary

| Plane | State at the probe | Exact identity |
| --- | --- | --- |
...
| Repository | `main` and `origin/main` pointed to source baseline `c2de415d` before this documentation-only reconciliation. The commit containing this page is a later documentation descendant; neither identity is the active consensus runtime. | Source baseline `c2de415d45dc3fb968542073d232e68b932b8e9f`; use `git rev-parse HEAD` for the moving documentation HEAD. |
| Adversarial verification | E1 is complete across all 10,240 generated trust graphs. E2 has a kickoff/status commit but no campaign implementation or evidence packet at `c2de415d`; E3-E6 have not started. | E1 completion `9ffa9992`; E2 kickoff `c2de415d`; [active milestone](../plans/active/cobalt-adversarial-verification-milestone.md). |
...
identity of the active validator binary. The `c2de415d` source baseline and this
later documentation-only reconciliation are also not deployed.
...
the off-chain E1 oracle/harness and evidence, and the E2 kickoff. None of those
facts means the source HEAD is installed on validators. A deployment claim
requires a service path, binary hash, and node observation; a source claim
requires a branch and commit; an experiment claim requires its frozen corpus and
result packet.
