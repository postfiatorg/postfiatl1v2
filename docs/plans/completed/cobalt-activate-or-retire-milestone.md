...
Consensus v2 also passes the paired integration gate. A 50-round baseline and 50-round Cobalt integration run started from the same signed six-validator state. The primary `consensus_round_ms` p95 moved from 1617.88 ms to 1660.42 ms, a 2.63% increase inside the 5% budget, while Cobalt covered 99.9985% of the integration window under the production 25% CPU quota. This 50+50-round local integration result is not comparable to E4's `wallet_to_finality_ms` p95 from a separate 500+500-round adversarial campaign. This is simulated protocol-capability evidence and requires no external operators.
...
- [x] Prove five-of-six progress, four-of-six rejection, canonical decision identity, signed catch-up, restart recovery, and no manual history repair. Code: [`cobalt_shadow.rs`](https://github.com/postfiatorg/postfiatl1v2/blob/main/crates/node/src/cobalt_shadow.rs), [`cobalt_shadow_runtime.rs`](https://github.com/postfiatorg/postfiatl1v2/blob/main/crates/node/src/cobalt_shadow_runtime.rs).
...
- [x] Write a versioned mathematical decision contract that defines compatible and incompatible trust configurations without importing or calling production Cobalt code. Evidence: [`oracle-contract.md`](https://github.com/postfiatorg/postfiatl1v2/blob/main/benchmarks/cobalt-activate-or-retire/oracle-contract.md).
- [x] Implement the oracle in a separate benchmark crate with hand-worked boundary fixtures and no production protocol dependency. Code: [`cobalt_decision_oracle`](https://github.com/postfiatorg/postfiatl1v2/tree/main/crates/cobalt_decision_oracle).
- [x] Replace permissive decisive expectations with a new frozen manifest containing a per-node prediction for every correct validator. The historical 80-case packet remains unchanged; the decisive generator is [`generate_inputs.py`](https://github.com/postfiatorg/postfiatl1v2/blob/main/benchmarks/cobalt-activate-or-retire/generate_inputs.py).
...
- [x] Add a production Cobalt adapter that reports every correct node’s decision and root plus certificate, timing, replay, authority, and rejection evidence. Code: [`postfiat_cobalt_decisive_benchmark.rs`](https://github.com/postfiatorg/postfiatl1v2/blob/main/crates/node/src/bin/postfiat_cobalt_decisive_benchmark.rs).
- [x] Add a pinned RippleD 3.1.3 adapter for the same proposed registry decision and each node’s local-UNL result, with native CSF ledger consensus separately labeled. Code: [`DecisiveGovernanceBenchmark_test.cpp`](https://github.com/postfiatorg/postfiatl1v2/blob/main/benchmarks/cobalt-activate-or-retire/rippled/DecisiveGovernanceBenchmark_test.cpp).
- [x] Freeze the oracle, 18 scenarios, source pins, both adapter hashes, and expected per-node results before execution. Manifest: [`scenario-manifest.json`](https://github.com/postfiatorg/postfiatl1v2/blob/main/benchmarks/cobalt-activate-or-retire/scenario-manifest.json), canonical ID `78fc3f92d460f45a4941d40ef705af6c761e3782155a5b599dbd78c90396bde3`.
...
- [x] Prove Consensus v2 continues finalizing in the controlled integration run, `consensus_round_ms` p95 stays within 5% of the frozen same-fleet baseline, and no Cobalt recovery mutates durable history by hand. Result: 50 rounds per lane, 1617.88 ms baseline `consensus_round_ms` p95, 1660.42 ms integration `consensus_round_ms` p95, +2.63%, 99.9985% Cobalt coverage, production-matched 25% CPU quota. These 50+50-round measurements are not comparable to E4's 500+500-round `wallet_to_finality_ms` campaign.
- [x] Produce signed-protocol, redaction-safe simulation receipts and a verifier-backed packet that identifies every result as simulated-validator evidence. Evidence: [`section3-packet`](https://github.com/postfiatorg/postfiatl1v2/tree/main/benchmarks/cobalt-activate-or-retire/section3-packet), `SHA256SUMS.txt` root `9a35119045698754ffdd11eea123bfad03bf2b3c23b700a55f0f539f5152bc18`. The packet makes no real provider, administrator, custody, geographic-independence, or decentralization claim.
