...
The pre-prove artifact is r4-construction/pre-prove-binding-check.json, schema postfiat.a666.r4.path-b-pre-prove-binding-check.v1. Its mandatory identity fields are candidate revision 39f7fae3191aa34c376ae1657650a9ec2444f421, a 64-lower-hex witness SHA-256, a readable repository-relative deployment evidence path plus its matching 64-lower-hex SHA-256, a 7-40-lower-hex deployment commit, and decoder SHA-256 6fb2fb9ac693e5cea787eeb4a80701de562d3b12ce47d837d121d8dc361f6d4a. Any reported counts or success booleans are supplemental only; fire-control recomputes the field and binding comparison.
...
The artifact must contain an ordered fields array with exactly those 35 canonical names. Every entry carries non-null expected_from_witness and decoded values; fire-control recomputes the equality of every pair and derives mismatch_count from the array. The deployment_bindings table has route, controller, wrapped, chain, genesis, protocol, program_vkey, and prior_checkpoint entries, each with actual decoded and deployed values; the aggregate recomputes every equality, including prior_checkpoint against the deployed initial checkpoint. Claimed booleans such as all_match or prior_checkpoint_matches_initial cannot make this gate GREEN. It also requires live_chain=false, business_value=0, and stakehub=false. An absent, malformed, stale, or mismatched record leaves pre_prove_all_35_bindings=false and blocks proving.
