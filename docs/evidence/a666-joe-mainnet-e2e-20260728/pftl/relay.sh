...
node=/opt/postfiat/releases/a666-joe-e2e-1312a94/postfiat-node
topology=/etc/postfiat/releases/a666-joe-e2e-1312a94/topology.json
run=/var/lib/postfiat/validator-2/a666-joe-e2e-20260728
...
deposit_tx=0x136512df7ea2764e1878c70d51ba85939596c1d18d159c4e21e162d0b83f7155
...
ssh_v2 "set -euo pipefail
  test -x /var/lib/postfiat/validator-2/pfusdc-latency-20260727-run2/cast
  ...
  '$node' vault-bridge-deposit-relay-rpc-bundle \
    --cast-bin /var/lib/postfiat/validator-2/pfusdc-latency-20260727-run2/cast \
...
ssh_v2 "set -euo pipefail
  ...
  jq -e '.assets|any(.balance == 101200000)' '$run/holder-after-claim.json' >/dev/null"
...
jq -n \
  ...
  '{schema:"postfiat.a666.joe_pfusdc_relay.v1",verdict:"PASS",deposit_tx:$deposit_tx,start_height:$start_height,finalized_height:$finalized_height,amount_atoms:100500000}' \
