...
**Status:** Deferred — the generic multi-asset NRRS is unfinished and is not
authorized for live value
...
partial A666 redemption -> pfUSDC sequence. The completed command sheet and
raw run payloads are retained in Git history rather than the active docs tree;
the [Evidence Index](../evidence/index.md) is the curated proof front door.
