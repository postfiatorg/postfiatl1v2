...
const CONFIG_SCHEMA = 'postfiat-a666-export-relay-config-v1';
...
function loadConfig(env, supplied) {
    const raw = supplied || (() => {
        const file = path.resolve(String(env.A666_EXPORT_RELAY_CONFIG_FILE || ''));
        if (!file) throw new Error('A666 export relay config is not configured');
        const stat = fs.lstatSync(file);
        if (!stat.isFile() || stat.isSymbolicLink() || stat.uid !== process.getuid()
            || (stat.mode & 0o022) !== 0 || stat.size > 64 * 1024) {
            throw new Error('A666 export relay config must be an owner-controlled regular file');
        }
        return JSON.parse(fs.readFileSync(file, 'utf8'));
    })();
    if (raw?.schema !== CONFIG_SCHEMA || raw.enabled !== true
        || raw.route_id !== ROUTE_ID || raw.route_config_digest !== ROUTE_DIGEST
        ...
        throw new Error('invalid A666 export relay config');
    }
    const driverBin = supplied
        ? path.resolve(raw.driver_bin)
        : securePinnedFile(raw.driver_bin, raw.driver_sha256, 'A666 export relay driver');
    const maxAmount = BigInt(String(raw.max_amount_atoms || '0'));
    if (maxAmount <= 0n) throw new Error('invalid A666 export relay amount cap');
    return {
        ...raw,
        ...
        inspect_timeout_ms: Number(raw.inspect_timeout_ms || 60_000),
        readiness_timeout_ms: Number(raw.readiness_timeout_ms || 90_000),
    };
}

function expandArgs(args, values) {
    return args.map((argument) => String(argument).replace(/\{([a-z_]+)\}/g, (_, key) => {
        if (!(key in values)) throw new Error(`unknown A666 relay placeholder: ${key}`);
        return values[key];
    }));
}
...
function canonicalJobId(packetHash) {
...
function normalizeRequest(body, config) {
    ...
    if (request.route_id !== ROUTE_ID || request.route_config_digest !== ROUTE_DIGEST
        ...
        throw Object.assign(new Error('invalid A666 export relay request'), {
            code: 'invalid_a666_export_relay_request',
        });
    }
    return request;
}
...
function validateInspection(request, inspection) {
    if (inspection?.ok !== true || inspection.route_id !== request.route_id
        ...
        throw Object.assign(new Error('A666 export packet inspection failed closed'), {
            code: 'a666_export_packet_binding_mismatch',
        });
    }
    return inspection;
}
...
function create(runtime = {}, options = {}) {
    ...
    if (!options.config && !env.A666_EXPORT_RELAY_CONFIG_FILE) {
        const unavailable = async () => ({
            ok: false,
            ready: false,
            route_id: ROUTE_ID,
            code: 'a666_export_relay_not_configured',
            message: 'A666 export relay is not configured on this wallet host.',
        });
        return {
            a666ExportRelayReadiness: unavailable,
            submitA666ExportRelayJob: async () => {
                throw Object.assign(new Error('A666 export relay is not configured.'), {
                    code: 'a666_export_relay_not_configured',
                });
            },
            a666ExportRelayJobStatus: () => null,
            closeA666ExportRelayJobs: () => {},
            canonicalA666ExportRelayJobId: canonicalJobId,
            _pumpA666ExportJobsForTest: () => {},
        };
    }
    const config = loadConfig(env, options.config);
    const root = path.resolve(options.root || env.A666_EXPORT_RELAY_JOB_ROOT
        || path.join(os.homedir(), '.postfiat', 'wallet-proxy-8080', 'a666-export-jobs-v1'));
    ...
    let pumping = false;
    const retryBaseMs = Number(options.retryBaseMs || env.A666_EXPORT_RELAY_RETRY_BASE_MS || 5_000);
    const retryMaxMs = Number(options.retryMaxMs || env.A666_EXPORT_RELAY_RETRY_MAX_MS || 300_000);
    ...
    function validateState(job, state) {
        ...
        if (!common || !terminal) throw new Error('invalid A666 export worker state');
        const publicFields = [
    ...
    function readJob(jobId) {
        ...
        if (job?.schema !== JOB_SCHEMA || job.job_id !== jobId) throw new Error('invalid A666 export job');
    ...
    async function refreshReadiness() {
        try {
            ...
            if (result?.ok !== true || result?.ready !== true || result.route_id !== ROUTE_ID
                ...
                || String(result.wrapped_token || '').toLowerCase() !== WRAPPED_TOKEN) {
                throw new Error('A666 export readiness identity mismatch');
            }
            readiness = result;
        } catch (error) {
            readiness = { ok: false, ready: false, route_id: ROUTE_ID,
                code: 'a666_export_relay_unavailable', message: 'A666 export relay is unavailable.',
    ...
    async function submitUnlocked(body) {
        ...
        const jobId = canonicalJobId(request.packet_hash);
        ...
        if (fs.existsSync(file)) {
            ...
            if (existing.request_fingerprint !== fingerprint) {
                throw Object.assign(new Error('A666 export job binding conflict'), { code: 'a666_export_job_binding_conflict' });
            }
            pump();
        ...
        atomicWrite(stateFileFor(jobId), { schema: STATE_SCHEMA, job_id: jobId,
            status: 'awaiting_source_debit', stage_index: 1, retryable: true, route_id: ROUTE_ID,
    ...
    return {
        a666ExportRelayReadiness: relayReadiness,
        submitA666ExportRelayJob: submit,
        a666ExportRelayJobStatus: status,
        closeA666ExportRelayJobs: close,
        canonicalA666ExportRelayJobId: canonicalJobId,
        _pumpA666ExportJobsForTest: pump,
    };
}
