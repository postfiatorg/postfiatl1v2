...
function word(value) {
  ...
  if (!/^[0-9a-f]*$/i.test(hex) || hex.length > 64) throw new Error('A666 return ABI value is invalid');
...
export function buildA666ReturnBurnCalldata({ amountAtoms, pftlRecipient, nativeNavAssetId, returnNonce }) {
  ...
  if (amount <= 0n || !/^pf[0-9a-f]{40}$/.test(recipient)
    ...
    throw new Error('A666 return burn fields are invalid');
  }
...
export function createA666ReturnNonce(cryptoApi = globalThis.crypto) {
...
async function relayJson(url, options = {}) {
  ...
  if (!response.ok || payload.ok !== true) {
    const error = new Error(payload.message || `A666 return relay failed with HTTP ${response.status}`);
...
export async function loadA666ReturnReadiness() {
  return relayJson('/api/a666/return-readiness');
}

export async function createA666ReturnJob({
  ...
  assertNoCustodyMaterial(body, 'A666 return relay request');
  return relayJsonWithRetry('/api/a666/return-jobs', {
    method: 'POST',
    headers: {
...
export async function loadA666ReturnJob(jobId) {
  return relayJsonWithRetry(`/api/a666/return-jobs/${encodeURIComponent(jobId)}`);
}

export async function waitForA666ReturnJob(
  jobId,
  ...
  while (Date.now() < deadline) {
    let result;
    try { result = await loadA666ReturnJob(jobId); } catch (error) {
    ...
    if (TERMINAL.has(result.status)) {
      ...
      const error = new Error(result.message || 'The trustless A666 return failed a safety gate.');
  ...
  error.code = 'a666_return_job_poll_timeout';
  throw error;
}
