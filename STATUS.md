...
- E4 exposed near-linear finality growth with chain height in the deployed JSON/JSONL lineage. Candidate `d0ae79f3` passed G4 but failed exact height-924 continuation. Successor `10dd9f20` fixed that defect and passed the old clone runner, but its first live canary exposed a missing concurrent transport/RPC topology and incomplete exact rollback proof. Validator-1 was restored to the old signed deployment, both candidates must not deploy, and public testnet remains blocked.
