...
pub const PFUSDC_ARC_INGRESS_PUBLIC_VALUES_SCHEMA_V1: &str =
...
const PFUSDC_ARC_INGRESS_CANONICAL_MAGIC_V1: &[u8] = b"PFTL-PFUSDC-ARC-INGRESS-V1";
...
/// Exact public input ABI emitted by `programs/pfusdc-arc-ingress`.
/// The fixed-width representation deliberately mirrors Tier-4 spec A4.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PfUsdcArcIngressPublicValuesV1 {
    pub schema: String,
    pub route_id: String,
    ...
    pub arc_block_hash: String,
    pub arc_block_height: u64,
    pub validator_set_commitment_in: String,
    pub validator_set_commitment_out: String,
}

impl PfUsdcArcIngressPublicValuesV1 {
    pub fn from_canonical_bytes(bytes: &[u8]) -> Result<Self, String> {
        const FIELD_BYTES: usize = 288;
        if bytes.len() != PFUSDC_ARC_INGRESS_CANONICAL_MAGIC_V1.len() + FIELD_BYTES
            || !bytes.starts_with(PFUSDC_ARC_INGRESS_CANONICAL_MAGIC_V1)
        {
            return Err("pfUSDC Arc ingress public values have wrong domain or length".to_string());
        }
        let mut offset = PFUSDC_ARC_INGRESS_CANONICAL_MAGIC_V1.len();
        let mut take = |len: usize| {
            let start = offset;
            offset += len;
            &bytes[start..offset]
        };
        let route_id = bytes_to_lower_hex(take(32));
        let arc_chain_id = u64::from_be_bytes(take(8).try_into().expect("fixed width"));
        let vault_address = format!("0x{}", bytes_to_lower_hex(take(20)));
        let token_address = format!("0x{}", bytes_to_lower_hex(take(20)));
        let deposit_id = bytes_to_lower_hex(take(32));
        let amount_atoms = u64::from_be_bytes(take(8).try_into().expect("fixed width"));
        let pftl_recipient_hash = bytes_to_lower_hex(take(32));
        let deposit_nonce = bytes_to_lower_hex(take(32));
        let arc_block_hash = bytes_to_lower_hex(take(32));
        let arc_block_height = u64::from_be_bytes(take(8).try_into().expect("fixed width"));
        let validator_set_commitment_in = bytes_to_lower_hex(take(32));
        let validator_set_commitment_out = bytes_to_lower_hex(take(32));
        let values = Self {
            schema: PFUSDC_ARC_INGRESS_PUBLIC_VALUES_SCHEMA_V1.to_string(),
            route_id,
            arc_chain_id,
            vault_address,
            token_address,
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.schema != PFUSDC_ARC_INGRESS_PUBLIC_VALUES_SCHEMA_V1
            || self.arc_chain_id == 0
            || self.arc_block_height == 0
            || self.amount_atoms == 0
        {
            return Err("pfUSDC Arc ingress public values have invalid schema or scalars".to_string());
        }
        validate_evm_address_text("pfusdc_arc_ingress.vault_address", &self.vault_address)?;
        validate_evm_address_text("pfusdc_arc_ingress.token_address", &self.token_address)?;
        pfusdc_validate_hex_fields(&[
            ("pfusdc_arc_ingress.route_id", &self.route_id, 64),
            ("pfusdc_arc_ingress.deposit_id", &self.deposit_id, 64),
            (
                "pfusdc_arc_ingress.pftl_recipient_hash",
                &self.pftl_recipient_hash,
                64,
            ),
            ("pfusdc_arc_ingress.deposit_nonce", &self.deposit_nonce, 64),
            ("pfusdc_arc_ingress.arc_block_hash", &self.arc_block_hash, 64),
            (
                "pfusdc_arc_ingress.validator_set_commitment_in",
                &self.validator_set_commitment_in,
                64,
            ),
            (
                "pfusdc_arc_ingress.validator_set_commitment_out",
                &self.validator_set_commitment_out,
                64,
            ),
        ])
    }
...
/// Governance-pinned Arc light-client state for one route epoch.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PfUsdcArcFinalityStateV1 {
    pub schema: String,
...
impl PfUsdcArcFinalityStateV1 {
    pub fn validate(&self) -> Result<(), String> {
        if self.schema != PFUSDC_ARC_FINALITY_STATE_SCHEMA_V1
            || self.route_epoch == 0
            || self.arc_chain_id == 0
            || self.latest_block_height == 0
        {
            return Err("pfUSDC Arc finality state has invalid schema or scalars".to_string());
        }
        validate_evm_address_text("pfusdc_arc_finality.vault_address", &self.vault_address)?;
        validate_evm_address_text("pfusdc_arc_finality.token_address", &self.token_address)?;
        for (field, value) in [
            (
                "pfusdc_arc_finality.vault_runtime_code_hash",
                &self.vault_runtime_code_hash,
            ),
            (
                "pfusdc_arc_finality.token_runtime_code_hash",
                &self.token_runtime_code_hash,
            ),
        ] {
            let raw = value.strip_prefix("0x").unwrap_or(value);
            validate_lower_hex_len(field, raw, 64)?;
        }
        pfusdc_validate_hex_fields(&[
            (
                "pfusdc_arc_finality.route_profile_hash",
                &self.route_profile_hash,
                96,
            ),
            ("pfusdc_arc_finality.route_binding", &self.route_binding, 64),
            (
                "pfusdc_arc_finality.latest_block_hash",
                &self.latest_block_hash,
                64,
            ),
            (
                "pfusdc_arc_finality.validator_set_commitment",
    ...
    pub fn verify_and_advance(
        &mut self,
        values: &PfUsdcArcIngressPublicValuesV1,
    ) -> Result<(), String> {
        self.validate()?;
        values.validate()?;
        if values.arc_chain_id != self.arc_chain_id
            || values.vault_address != self.vault_address
            || values.token_address != self.token_address
            || values.route_id != self.route_binding
            || values.validator_set_commitment_in != self.validator_set_commitment
        {
            return Err("pfUSDC Arc proof does not match its pinned route state".to_string());
        }
        if values.arc_block_height <= self.latest_block_height {
            return Err("pfUSDC Arc proof does not monotonically advance finality".to_string());
        }
        self.latest_block_height = values.arc_block_height;
        self.latest_block_hash = values.arc_block_hash.clone();
        self.validator_set_commitment = values.validator_set_commitment_out.clone();
        self.validate()
    }

    pub fn state_commitment_bytes(&self) -> Result<Vec<u8>, String> {
        self.validate()?;
        let mut out = pfusdc_canonical_prefix(PFUSDC_ARC_FINALITY_STATE_SCHEMA_V1);
        pfusdc_append_text(&mut out, 1, &self.schema)?;
        pfusdc_append_hex(&mut out, 2, &self.route_profile_hash)?;
        pfusdc_append_u64(&mut out, 3, self.route_epoch);
        pfusdc_append_u64(&mut out, 4, self.arc_chain_id);
        pfusdc_append_evm_address(&mut out, 5, &self.vault_address)?;
        pfusdc_append_hex(
            &mut out,
            6,
            self.vault_runtime_code_hash
        ...
        pfusdc_append_evm_address(&mut out, 7, &self.token_address)?;
        pfusdc_append_hex(
            &mut out,
            8,
            self.token_runtime_code_hash
        ...
        pfusdc_append_hex(&mut out, 9, &self.route_binding)?;
        pfusdc_append_u64(&mut out, 10, self.latest_block_height);
        pfusdc_append_hex(&mut out, 11, &self.latest_block_hash)?;
        pfusdc_append_hex(&mut out, 12, &self.validator_set_commitment)?;
        Ok(out)
    }
}
