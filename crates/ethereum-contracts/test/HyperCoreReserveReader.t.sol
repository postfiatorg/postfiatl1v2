...
contract HyperCoreReserveReaderTest {
    ...
    function testRejectsZeroAccountBeforeReadingPrecompiles() public {
        ...
        HyperCoreReserveReader.SpotRequest[] memory spots =
            new HyperCoreReserveReader.SpotRequest[](0);
        (bool ok,) = address(reader).call(
            abi.encodeCall(reader.snapshot, (address(0), perps, spots, 0, bytes32(uint256(1))))
        );
    ...
    function testRejectsDuplicateOrUnsortedPerpSetBeforeReadingPrecompiles() public {
        ...
        HyperCoreReserveReader.SpotRequest[] memory spots =
            new HyperCoreReserveReader.SpotRequest[](0);
        (bool ok,) = address(reader).call(
            abi.encodeCall(reader.snapshot, (address(1), perps, spots, 0, bytes32(uint256(1))))
        );
    ...
    function testRejectsDuplicateOrUnsortedSpotSetBeforeReadingPrecompiles() public {
        uint32[] memory perps = new uint32[](0);
        HyperCoreReserveReader.SpotRequest[] memory spots =
            new HyperCoreReserveReader.SpotRequest[](2);
        spots[0] = HyperCoreReserveReader.SpotRequest({
            token: 404,
            weiDecimals: 8,
            priceAsset: 224,
            priceAssetSzDecimals: 3
        });
        spots[1] = spots[0];
        (bool ok,) = address(reader).call(
            abi.encodeCall(reader.snapshot, (address(1), perps, spots, 0, bytes32(uint256(1))))
