...
pub(super) fn run_rpc(flags: &[String]) -> Result<(), String> {
    ...
    match method {
        ...
        "yolo_target_receipt" => {
            let registration_id =
                flag_value(flags, "--registration-id").ok_or("missing --registration-id")?;
            let report =
                postfiat_node::yolo_target_receipt_query(NodeOptions { data_dir }, registration_id)
