...
claim_route_epoch=$(jq -er '.route.route_epoch' "$pfusdc_deployment_manifest")
claim_asset=$(jq -er '.settlement_source_asset_id // .settlement_asset_id' "$ops_dir/manifest.json")
[[ "$manifest_vault" =~ ^0x[0-9a-f]{40}$ ]]
[[ "$claim_policy_hash" =~ ^[0-9a-f]{96}$ ]]
[[ "$claim_route_epoch" =~ ^[0-9]+$ ]]
...
PFTL_ROUTE_EPOCH="$claim_route_epoch" \
PFTL_CLAIM_ASSET_ID="$claim_asset" \
