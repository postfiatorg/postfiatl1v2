# Deployment day — combined-devnet-20260923
...
checkout containing this packet and `deployments/combined-devnet-20260921/`
(the helpers reuse its scripts). Deploy only the qualified node executable
(`e7bb1afa…`, build `1a0989ad`). This release replaces the plan to deploy
combined-devnet-20260921 first: the six validators go directly from
`a666-source-route-20260907` to this release in one rollout. 20260921 is the
prepared fallback only.
...
Sign the stage using [SIGNING.md](SIGNING.md), path (a) (the other lane, existing
deployment and snapshot publisher keys) or path (b) (approved publisher-key
rotation; then use its `STAGE`, `PACKET` and `$KEYDIR` snapshot files throughout
this sheet). The path (a) manifest command is:
...
## 1a. Unattended-upgrade window — check before starting
...
`docs/status/chain-state-current.md` on main (`b8b5130c`). `apt-daily-upgrade`
runs on each host daily at a random time between about 06:00 and 07:00Z.
**Do not start the rollout inside that window.** Before step 2, read the timer on
every host (read-only):

```bash
for host in 64.176.220.75 95.179.184.122 66.42.48.39 149.28.63.106 95.179.179.206 45.32.110.170; do
  echo "== $host"
...
Start only when every host's `NEXT` run is later than the expected end of the
whole rollout (all six applies plus final observation), and no
`apt-daily-upgrade.service` or needrestart run is active. If a `NEXT` falls
inside the rollout, wait until it has run and its restarts have settled, then
capture a fresh baseline. Do not disable or edit the timers as part of this
release.
...
signing command and supported resume command are in the
...
```text
/var/lib/postfiat/rollback/combined-devnet-20260923/validator-N
```
...
The September 18 and 22 local rollback rehearsals
([20260918](../release-repair-20260918/README.md),
[20260922](../release-repair-20260922/README.md#certified-history-and-rollback-scope))
establish compatibility, not the
...
build `1a0989ad` and the new binary/manifest/artifact hashes; remaining nodes
