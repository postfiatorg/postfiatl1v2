...
pub const ROUTE_ID: &str = "ethereum-sepolia-usdc-v1";
pub const SEPOLIA_CHAIN_ID: u64 = 11_155_111;
pub const SEPOLIA_GENESIS_VALIDATORS_ROOT: B256 = B256::new([
...
pub fn verify_witness(w: &EthIngressWitnessV1) -> Result<EthIngressPublicValuesV1, String> {
    ...
    if p.route_id != ROUTE_ID
        || p.source_chain_id != SEPOLIA_CHAIN_ID
        || p.genesis_validators_root != SEPOLIA_GENESIS_VALIDATORS_ROOT
        || p.manifest_hash == B256::ZERO
        || p.vault_runtime_code_hash == B256::ZERO
        || p.token_runtime_code_hash == B256::ZERO
    {
        return Err("Ethereum ingress policy is not the allowlisted Sepolia P0 route".into());
    }
    w.evidence.validate()?;
...
fn verify_finality(inputs: &ProofInputs, p: &EthIngressPolicyV1) -> Result<Finality, String> {
    ...
    {
        return Err("Helios input does not match pinned Sepolia policy".into());
    }
    require_sepolia_forks(&inputs.forks)?;
    ...
    if final_slot <= prior_slot {
        return Err("Sepolia finalized checkpoint did not canonically advance".into());
    }
    let execution = store
        .finalized_header
        .execution()
        .map_err(|_| "finalized Sepolia header has no execution payload".to_string())?;
    Ok(Finality {
        prior_root,
        prior_slot,
...
fn require_sepolia_forks(f: &helios_consensus_core::types::Forks) -> Result<(), String> {
    let expected = [
        (&f.genesis, 0, [0x90, 0, 0, 0x69]),
        (&f.altair, 50, [0x90, 0, 0, 0x70]),
        (&f.bellatrix, 100, [0x90, 0, 0, 0x71]),
        (&f.capella, 56_832, [0x90, 0, 0, 0x72]),
        (&f.deneb, 132_608, [0x90, 0, 0, 0x73]),
        (&f.electra, 222_464, [0x90, 0, 0, 0x74]),
        (&f.fulu, 272_640, [0x90, 0, 0, 0x75]),
    ];
    if expected.iter().any(|(fork, epoch, version)| {
        fork.epoch != *epoch || fork.fork_version.as_slice() != version
    }) {
        return Err("Helios fork schedule does not match Sepolia".into());
    }
    Ok(())
}
