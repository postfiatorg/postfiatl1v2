...
| Storage scaling | Current source contains an undeployed transactional `redb` finality-path candidate. Snapshot/rebuild equality and the exact height-915 archive replay pass; exact height 924, the complete tamper matrix, paired six-validator performance, and six-clone migration remain open. Public testnet is blocked. See the [active milestone](docs/plans/active/storage-scaling-milestone.md). |
