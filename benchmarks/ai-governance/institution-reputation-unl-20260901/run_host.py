#!/usr/bin/env python3
"""Run one fixed-batch reputation pass against the local pinned SGLang server."""
...
def validate_result(request_row: dict[str, Any], content: str) -> dict[str, Any]:
    ...
    for field in ("validator_id", "network", "entity"):
        if parsed.get(field) != request_row[field]:
            raise ValueError(f"slot {request_row['slot']}: {field} mismatch")
