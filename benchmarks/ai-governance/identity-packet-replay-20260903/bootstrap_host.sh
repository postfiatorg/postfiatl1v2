#!/bin/bash
# Launch the pinned Qwen3.8 deterministic SGLang profile on one H200-class host.
# Loopback only; radix cache disabled so every request takes the fresh-prefill path.
set -euo pipefail
cd /root/identity-replay
...
open("/root/identity-replay/model_path.txt", "w").write(path)
print("model ready")
PY
