...
This is a SHADOW_ONLY measurement. Do not use validator uptime, consensus performance, hardware, or list membership to raise the score. Score only the institution behind the validator.

You receive one frozen validator identity packet: cited public-identity research that names the canonical institution the packet concludes stands behind the validator key, with its evidence and its unresolved uncertainty. Read the whole packet, including its Uncertainty and Conflicts section and its Machine-Readable Summary.

How to use the packet:
- The packet establishes WHICH institution to score. Score the canonical entity the packet identifies, not the domain string and not the validator key.
- The packet does not establish legitimacy or prestige by itself. Its length, its number of citations, and the confidence it states must not raise the score.
- Mandatory recognition rule: after identifying the canonical entity, ask whether you genuinely recognize that institution from your own model knowledge. If you do not recognize it, if the packet reports that no institution could be established, if the packet's canonical entity is an individual, pseudonym, or hobby operation rather than an institution, or if the claimed domain and the institution do not plausibly belong together, set recognized=false and score=0. Do not guess, infer legitimacy from a plausible name, or give an obscure or unrecognized entity partial credit.
- If the packet reports material conflicts about which entity controls the validator, score conservatively and say why.
...
{BANDS}
Choose an integer score inside the supported band. Set entity to the canonical institution you scored (or null when unrecognized), and copy validator_id, network, and packet_sha256 exactly from the request header. Return the explanation as an explanation_paragraphs array containing two or three short paragraphs. State the independent facts you genuinely remember about the institution, how the packet's identity conclusion and uncertainty affected the score, the sanctions/integrity assessment, and the value or harm the institution would bring to a Layer-1. Do not claim to have browsed or performed a live sanctions search.
...
def build() -> None:
    ...
    for slot, row in enumerate(packets):
        header = {
            ...
            "corpus": CORPUS_NAME,
            "corpus_packet_set_sha256": corpus_hashes["packet_set_sha256"],
        }
        user = (
            "Score the institution behind this validator. Request header (copy validator_id, network, packet_sha256 exactly):\n"
            + canonical(header)
            + "\n\nFrozen identity packet (exact bytes; SHA-256 above):\n\n"
            + row["packet_markdown"]
        )
        body = {
            "model": MODEL,
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {
                    "name": "institution_reputation_packet",
                    "strict": True,
    ...
    manifest = {
        "artifact": "institution-reputation-packets-20260903",
        "predecessor": "institution-reputation-unl-20260901",
