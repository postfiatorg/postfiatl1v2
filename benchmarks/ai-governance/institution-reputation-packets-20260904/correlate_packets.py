        ...
        rows.append(
            {
                ...
                "claimed_domain": entry.get("claimed_domain"),
                "registrable_domain": registrable(entry.get("claimed_domain")),
                "official_hosts": sorted({host(u) for u in ([summary.get("official_website")] if summary.get("official_website") else []) if host(u)}),
...
def main() -> None:
    ...
    report = {
        "artifact": "institution-reputation-packets-20260904/correlation",
        "shadow_only": True,
        "method": "deterministic pairwise overlap of profile Machine-Readable Summary fields plus the input claimed domain; no model, no network",
        "identity_corpus": {
            "name": "validator-identity-packets-20260904",
    ...
    lines = [
        ...
        f"frozen packet in `validator-identity-packets-20260904` (packet-set SHA-256 `{report['identity_corpus']['packet_set_sha256']}`).",
