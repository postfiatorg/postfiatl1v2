#!/bin/bash
# Bootstrap a rented H200 host: pinned model download + deterministic SGLang launch.
set -euo pipefail
cd /root/rep-run
...
p = snapshot_download("Qwen/Qwen3.8-27B-FP8", revision="$REV", local_files_only=False)
open("/root/rep-run/model_path.txt", "w").write(p)
print("model at", p)
PY
...
for i in $(seq 1 120); do
  if curl -sf http://127.0.0.1:8000/health >/dev/null 2>&1; then echo READY; exit 0; fi
  sleep 10
done
echo "server failed to become healthy" >&2
tail -40 sglang.log >&2
exit 1
