...
use pfusdc_eth_ingress_program::{
    verify_witness, EthIngressPolicyV1, EthIngressWitnessV1, POLICY_SCHEMA, ROUTE_ID,
    SEPOLIA_CHAIN_ID, SEPOLIA_GENESIS_VALIDATORS_ROOT, WITNESS_SCHEMA,
};
use postfiat_types::{
...
async fn capture(
    ...
    let (helios, final_block, final_hash) = loop {
        ...
        eprintln!(
            "waiting for Sepolia finality: deposit block {deposit_block}, finalized {}",
            e.block_number()
        );
    ...
    let mut manifest = b"postfiat.ethereum-sepolia-usdc-v1.p0\0".to_vec();
    ...
    println!("captured finalized Sepolia deposit block {deposit_block} under finalized block {final_block}; deposit {deposit_id_for_report}");
    Ok(())
}
...
async fn capture_helios(rpc: &Rpc, beacon: &str) -> Result<ProofInputs> {
    ...
    Ok(ProofInputs {
        ...
        finality_update,
        store,
        genesis_root: SEPOLIA_GENESIS_VALIDATORS_ROOT,
        forks,
...
fn sepolia_forks() -> Forks {
    ...
    Forks {
        ...
        capella: f(56832, [0x90, 0, 0, 0x72]),
        deneb: f(132608, [0x90, 0, 0, 0x73]),
        electra: f(222464, [0x90, 0, 0, 0x74]),
        fulu: f(272640, [0x90, 0, 0, 0x75]),
    }
}
async fn get_proof(
    r: &Rpc,
...
async fn find_balance_slot(
    ...
    bail!("Circle Sepolia USDC balance slot not found")
}
