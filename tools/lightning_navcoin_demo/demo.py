...
from .crash_matrix import crash_service_transition, run_crash_matrix
...
class SyntheticDemo:
    ...
    def live_crash_recovery(self) -> QuoteContext:
        """Crash after every coordinator edge while reconciling real effects."""
        ...
        known_secret = context.invoice_secret_known_to_coordinator
        if known_secret is None:
            raise DemoFailure("live crash probe requires coordinator invoice control")
        root = (
            self.devnet.root
            / "private"
            / "demo-runs"
        ...
        crash(
            "ADMIT",
            {"coordinator_secret_hex": known_secret.protocol_hex()},
        ...
        channels_before = _channel_state(self.lnd)
        bitcoin_height = int(self.lnd.get_info("user")["block_height"])
        payment = self.lnd.pay_invoice(
            "user",
            str(context.quote["invoice"]),
            fee_limit_sat=20,
        ...
        if payment.payment_preimage is None:
            raise DemoFailure("live crash payment returned no preimage")
        settlement = assert_lightning_settlement(
            LightningSettlement(
                payment_hash=payment.payment_hash,
                payment_preimage=payment.payment_preimage.reveal_for_protocol(),
    ...
    def finalize(self) -> Path:
        ...
        summary = {
            ...
            "adversarial": {
                ...
                "duplicate_finish": "REJECTED_MUTATION_FREE",
                "duplicate_cancel": "REJECTED_MUTATION_FREE",
                "amp": "REJECTED_BEFORE_PAYMENT",
                "route_failure": "NO_VALUE_DELTA",
                "crash_every_transition": "REAL_EFFECTS_RECONCILED",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "verify-evidence":
            print(json.dumps(verify_bundle(arguments.evidence_dir), indent=2, sort_keys=True))
            return 0
