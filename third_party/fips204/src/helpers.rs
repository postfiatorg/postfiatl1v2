...
pub(crate) fn mat_vec_mul<const K: usize, const L: usize>(
    ...
    crate::cycle_tracking::write(b"cycle-tracker-report-start:mldsa.pointwise.matvec\n");
    let mut w_hat = [T0; K];
    let u_hat_mont = to_mont(u_hat);
    for i in 0..K {
        #[cfg(feature = "riscv32-verify-arithmetic")]
        for n in 0..256 {
            // Montgomery reduction is linear modulo q. Accumulating the bounded
            // K-by-L dot product in i64 and reducing once removes L-1 reductions
            // and L-1 output load/stores per coefficient. ML-DSA's largest
            // parameter set has L=7, so the sum remains far inside Algorithm
            // 49's signed input bound.
            let mut dot_product = 0i64;
            #[allow(clippy::needless_range_loop)] // fixed-size protocol parameter
            for j in 0..L {
                dot_product += i64::from(a_hat[i][j].0[n]) * i64::from(u_hat_mont[j].0[n]);
            }
            w_hat[i].0[n] = mont_reduce(dot_product);
        }
        #[cfg(not(feature = "riscv32-verify-arithmetic"))]
        #[allow(clippy::needless_range_loop)] // match the pinned upstream implementation
        for j in 0..L {
    ...
    crate::cycle_tracking::write(b"cycle-tracker-report-end:mldsa.pointwise.matvec\n");
    w_hat
}
...
pub(crate) fn to_mont<const L: usize>(vec_a: &[T; L]) -> [T; L] {
    #[cfg(feature = "riscv32-verify-arithmetic")]
    {
        // R^2 mod q for R=2^32. One Montgomery reduction computes
        // x*R^2*R^-1 = x*R (mod q), replacing the multi-step 64-bit Barrett
        // reduction previously used for x<<32. This is materially cheaper on
        // SP1's 32-bit RISC-V target.
        const R2_MOD_Q: i64 = 2_365_951;
        core::array::from_fn(|l| {
            T(core::array::from_fn(|n| mont_reduce(i64::from(vec_a[l].0[n]) * R2_MOD_Q)))
        })
    }
    #[cfg(not(feature = "riscv32-verify-arithmetic"))]
    {
...
mod tests {
    ...
    fn to_mont_reference<const L: usize>(values: &[T; L]) -> [T; L] {
        core::array::from_fn(|l| {
            T(core::array::from_fn(|n| partial_reduce64(i64::from(values[l].0[n]) << 32)))
        })
    }
    ...
    #[cfg(feature = "riscv32-verify-arithmetic")]
    #[test]
    fn optimized_to_mont_is_reference_equivalent_mod_q() {
        let inputs = [T(core::array::from_fn(|index| {
            let magnitude = i32::try_from(index).unwrap() * 131_071;
            if index % 2 == 0 {
                magnitude
            } else {
                -magnitude
        ...
        let optimized = to_mont(&inputs);
        let reference = to_mont_reference(&inputs);
        for (actual, expected) in optimized[0].0.iter().zip(reference[0].0.iter()) {
            assert_eq!(actual.rem_euclid(Q), expected.rem_euclid(Q));
        }
    }

    #[cfg(feature = "riscv32-verify-arithmetic")]
    #[test]
    fn accumulated_pointwise_reduction_is_reference_equivalent_mod_q() {
        const K: usize = 6;
        const L: usize = 5;
        const Q_USIZE: usize = 8_380_417;
        let matrix: [[T; L]; K] = core::array::from_fn(|row| {
            core::array::from_fn(|column| {
                T(core::array::from_fn(|coefficient| {
                    i32::try_from((row * 977 + column * 313 + coefficient * 17) % Q_USIZE).unwrap()
                }))
            })
        });
        let vector: [T; L] = core::array::from_fn(|column| {
            T(core::array::from_fn(|coefficient| {
                i32::try_from((column * 733 + coefficient * 29) % Q_USIZE).unwrap()
            }))
        });
        let optimized = mat_vec_mul(&matrix, &vector);
        let vector_mont = to_mont_reference(&vector);
        for row in 0..K {
            for coefficient in 0..256 {
                let mut reference = 0i32;
                for column in 0..L {
                    reference += mont_reduce(
                        i64::from(matrix[row][column].0[coefficient])
                            * i64::from(vector_mont[column].0[coefficient]),
                    );
                }
                assert_eq!(optimized[row].0[coefficient].rem_euclid(Q), reference.rem_euclid(Q));
            }
        }
    }
}
