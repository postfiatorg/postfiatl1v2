...
pub enum Error {
    ...
    /// A pinned verifier-key artifact is malformed or does not match the circuit shape.
    InvalidPinnedVerificationKey(&'static str),
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::InvalidPinnedVerificationKey(message) => {
                write!(f, "Invalid pinned verification key: {message}")
            }
        }
    }
}
